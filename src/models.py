...
class LSTMForecaster(BaseForecaster):
    ...
    def predict(self, X):
        ...
        # Pad lại phần đầu bằng NaN để khớp độ dài với X ban đầu
        # Vì seq_length điểm dữ liệu đầu tiên không đủ để tạo 1 sequence
        pad_width = len(X) - len(pred_values)
        full_preds = np.pad(pred_values, (pad_width, 0), mode='constant', constant_values=np.nan)
